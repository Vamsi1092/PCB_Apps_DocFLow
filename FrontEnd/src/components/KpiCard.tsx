...
export function KpiCard({ icon: Icon, value, label, trend, accent = 'default', onClick }: KpiCardProps) {
  ...
  // A trend with no digit (e.g. "Not available") isn't a real up/down movement —
  // show it as plain muted text instead of a misleading directional arrow.
  const hasTrendValue = /\d/.test(trend);
  ...
  return (
    ...
    >
      <div className="mb-[13px] flex items-center justify-between">
        ...
        <span
          className={cn('flex items-center gap-[2px] text-[11.5px]', hasTrendValue && a.trendWeight)}
          style={{ color: hasTrendValue ? a.trend : 'var(--faint)' }}
        >
          {hasTrendValue && <TrendIcon size={11} strokeWidth={2.75} />}
          {trendText}
        </span>
      </div>
      <div
      ...
      <div className="mt-[5px] text-[12.5px] font-medium uppercase tracking-[.04em] text-muted-foreground">{label}</div>
    </div>
  );
}
