...
function defaultSlaPolicies(): SlaPolicy[] {
  return stages.map((s) => ({ stage: s, targetHours: DEFAULT_SLA_HOURS[s] ?? 4, escalate: true }));
}
  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        ...
        {footer && <div className="flex items-center justify-end gap-2 border-t border-borderf px-5 py-3.5">{footer}</div>}
      </div>
    </div>
  );
}
...
export default function SettingsPage() {
  ...
  // Every section persists locally so a refresh doesn't lose edits. This is
  // still local-only demo configuration — see the page-level note below.
  const [tiers, setTiers] = useLocalStorage<ApprovalTier[]>('docflow.settings.tiers', INITIAL_TIERS);
  const [glMappings, setGlMappings] = useLocalStorage<GlMapping[]>('docflow.settings.glMappings', INITIAL_GL_MAPPINGS);
  const [glQuery, setGlQuery] = useState('');
  const [newGl, setNewGl] = useState({ vendor: '', accountCode: '', accountName: '' });
  const [slaPolicies, setSlaPolicies] = useLocalStorage<SlaPolicy[]>('docflow.settings.slaPolicies', defaultSlaPolicies);
  const [integrations, setIntegrations] = useLocalStorage<Integration[]>('docflow.settings.integrations', INITIAL_INTEGRATIONS);
  const [syncingId, setSyncingId] = useState<string | null>(null);
  const [team, setTeam] = useLocalStorage<TeamMember[]>('docflow.settings.team', INITIAL_TEAM);
  const [notifyPrefs, setNotifyPrefs] = useLocalStorage('docflow.settings.notifyPrefs', INITIAL_NOTIFY_PREFS);

  const close = () => setOpenModal(null);

  const flashSaved = (message: string) => showToast(message);
  ...
  const removeTier = (id: string) => {
    if (window.confirm('Remove this approval tier?')) setTiers((prev) => prev.filter((t) => t.id !== id));
  };
  const addTier = () => setTiers((prev) => [...prev, { id: nextId('t'), upTo: null, approver: 'Approver' }]);
  const resetTiers = () => {
    if (window.confirm('Reset approval thresholds to default?')) setTiers(INITIAL_TIERS);
  };
  // Non-negative check only — a limit of 0 or a negative number can't mean
  // anything real, so flag it without blocking further typing.
  const tierErrors = useMemo(() => {
    const errs: Record<string, string> = {};
    tiers.forEach((t) => {
      if (t.upTo != null && (isNaN(t.upTo) || t.upTo < 0)) errs[t.id] = 'Must be 0 or greater';
    });
    return errs;
  }, [tiers]);

  // ---- GL Mapping ----
  const removeGlMapping = (id: string) => {
    if (window.confirm('Remove this GL mapping?')) setGlMappings((prev) => prev.filter((g) => g.id !== id));
  };
  const updateGlMapping = (id: string, patch: Partial<GlMapping>) => {
    setGlMappings((prev) => prev.map((g) => (g.id === id ? { ...g, ...patch } : g)));
  };
  const resetGlMappings = () => {
    if (window.confirm('Reset GL mappings to default?')) setGlMappings(INITIAL_GL_MAPPINGS);
  };
  ...
  const newGlDuplicate = newGl.vendor.trim() !== ''
    && glMappings.some((g) => g.vendor.trim().toLowerCase() === newGl.vendor.trim().toLowerCase());
  const addGlMapping = () => {
    if (!newGl.vendor.trim() || newGlDuplicate) return;
    setGlMappings((prev) => [...prev, { id: nextId('g'), vendor: newGl.vendor.trim(), accountCode: newGl.accountCode.trim(), accountName: newGl.accountName.trim() }]);
    setNewGl({ vendor: '', accountCode: '', accountName: '' });
  };
  ...
  const resetSlaPolicies = () => {
    if (window.confirm('Reset SLA policies to default?')) setSlaPolicies(defaultSlaPolicies());
  };
  ...
  // Simulated only — no real network/ERP call. `aria-live` region below
  // announces the state change for screen readers.
  ...
  const removeTeamMember = (id: string) => {
    if (window.confirm('Remove this team member?')) setTeam((prev) => prev.filter((m) => m.id !== id));
  };
  const addTeamMember = () => {
  ...
  const resetTeam = () => {
    if (window.confirm('Reset team & roles to default?')) setTeam(INITIAL_TEAM);
  };
  const teamErrors = useMemo(() => {
    const errs: Record<string, string> = {};
    team.forEach((m) => {
      const email = m.email.trim().toLowerCase();
      if (!email || !EMAIL_RE.test(email)) { errs[m.id] = 'Invalid email format'; return; }
      const dup = team.some((other) => other.id !== m.id && other.email.trim().toLowerCase() === email);
      if (dup) errs[m.id] = 'Duplicate email';
    });
    return errs;
  }, [team]);
  ...
  const resetNotifyPrefs = () => {
    if (window.confirm('Reset notification preferences to default?')) setNotifyPrefs(INITIAL_NOTIFY_PREFS);
  };
  ...
  return (
    <div className="pcb-view">
      ...
      {openModal === 'thresholds' && (
          ...
          footer={(
            <>
              <button
                type="button"
                onClick={resetTiers}
                className="pcb-btn mr-auto flex h-[34px] items-center gap-1.5 rounded-lg border border-line bg-surface px-3 text-[12px] font-semibold text-text2"
              >
                <RotateCcw size={13} />Reset to default
              </button>
              <button
                type="button"
                onClick={() => { flashSaved('Approval thresholds saved locally'); close(); }}
                disabled={Object.keys(tierErrors).length > 0}
                className="pcb-btn h-[34px] rounded-lg border-none bg-navy px-4 text-[12.5px] font-semibold text-white disabled:opacity-40"
              >
                Save changes
              </button>
            </>
        ...
        >
          <div className="space-y-2.5">
            {tiers.map((t, i) => (
              <div key={t.id}>
                <div className="flex items-center gap-2.5 rounded-lg border border-border2 p-2.5">
                  ...
                  />
                  <button type="button" onClick={() => removeTier(t.id)} aria-label="Remove tier" className="pcb-btn flex-none rounded-md p-1.5 text-muted-foreground hover:text-destructive">
                    <Trash2 size={15} />
                  </button>
                </div>
                {tierErrors[t.id] && <p className="mt-1 pl-9 text-[11px] font-medium" style={{ color: RED }}>{tierErrors[t.id]}</p>}
              </div>
            ))}
          </div>
          <button
      ...
      {openModal === 'gl' && (
        <Modal
          ...
          footer={(
            <>
              ...
              <button
                type="button"
                onClick={() => { flashSaved('GL mappings saved locally'); close(); }}
        ...
        >
          <input
            ...
            aria-label="Search vendor or account"
            ...
            {filteredGl.map((g) => (
              <div key={g.id} className="flex items-center gap-2.5 rounded-lg border border-border2 p-2.5">
                ...
                <input
                  ...
                  aria-label={`Account code for ${g.vendor}`}
                ...
                <input
                  ...
                  aria-label={`Account name for ${g.vendor}`}
                  className="h-8 flex-1 rounded-md border border-line bg-surface px-2.5 text-[12.5px]"
                />
                <button type="button" onClick={() => removeGlMapping(g.id)} aria-label={`Remove mapping for ${g.vendor}`} className="pcb-btn flex-none rounded-md p-1.5 text-muted-foreground hover:text-destructive">
                  <Trash2 size={15} />
                </button>
              </div>
            ))}
          ...
          <div className="mt-3 rounded-lg border border-dashed border-line p-2.5">
            ...
            <div className="flex items-center gap-2">
              <input
                type="text"
                value={newGl.vendor}
                onChange={(e) => setNewGl((v) => ({ ...v, vendor: e.target.value }))}
                placeholder="Vendor"
                aria-invalid={newGlDuplicate}
                className="h-8 w-[170px] flex-none rounded-md border border-line bg-surface px-2 text-[12.5px]"
              />
              <input
                type="text"
                value={newGl.accountCode}
                onChange={(e) => setNewGl((v) => ({ ...v, accountCode: e.target.value }))}
                placeholder="Code"
                className="h-8 w-[70px] flex-none rounded-md border border-line bg-surface px-2 text-[12.5px] tabular-nums"
              />
              <input
                type="text"
                value={newGl.accountName}
                onChange={(e) => setNewGl((v) => ({ ...v, accountName: e.target.value }))}
              ...
              <button
                ...
                disabled={!newGl.vendor.trim() || newGlDuplicate}
                className="pcb-btn flex h-8 flex-none items-center gap-1 rounded-md border-none bg-navy px-3 text-[12px] font-semibold text-white disabled:opacity-40"
              >
                <Plus size={13} />Add
              </button>
            </div>
            {newGlDuplicate && <p className="mt-1.5 text-[11px] font-medium" style={{ color: RED }}>A mapping for this vendor already exists.</p>}
          </div>
        </Modal>
      )}
        ...
        <Modal
          ...
          footer={(
            <>
              ...
              <button
                type="button"
                onClick={() => { flashSaved('SLA policies saved locally'); close(); }}
                className="pcb-btn h-[34px] rounded-lg border-none bg-navy px-4 text-[12.5px] font-semibold text-white"
              >
                Save changes
              </button>
              ...
              <div key={p.stage} className="flex items-center gap-3 rounded-lg border border-border2 p-2.5">
                ...
                <div className="flex items-center gap-1.5 text-[12.5px] text-muted-foreground">
                  <input
                    ...
                    aria-label={`Target hours for ${p.stage}`}
      ...
      {openModal === 'integrations' && (
        <Modal
          title="Integrations"
          subtitle="ERP sync (NetSuite), email capture, and EDI gateway connections — all simulated, no real connections are made"
          onClose={close}
        >
          <div className="space-y-2.5">
            {integrations.map((i) => (
              <div key={i.id} className="flex items-center gap-3 rounded-lg border border-border2 p-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 text-[11px] text-faint" role="status" aria-live="polite">
                    {syncingId === i.id ? 'Syncing (simulated)…' : i.connected ? `Last synced ${i.lastSynced}` : 'Disconnected'}
                  </div>
                </div>
                <button
                  type="button"
        ...
        <Modal
          ...
          footer={(
            <>
              ...
              <button
                type="button"
                onClick={() => { flashSaved('Team changes saved locally'); close(); }}
                disabled={Object.keys(teamErrors).length > 0}
                className="pcb-btn h-[34px] rounded-lg border-none bg-navy px-4 text-[12.5px] font-semibold text-white disabled:opacity-40"
              >
                Save changes
              </button>
                ...
                <div className="flex items-center gap-2.5 rounded-lg border border-border2 p-2.5">
                  <div className="min-w-0 flex-1">
                    ...
                    <input
                      ...
                      aria-invalid={!!teamErrors[m.id]}
                      className="mt-0.5 h-7 w-full rounded-md border border-line bg-surface px-2 text-[11.5px] text-muted-foreground"
                    />
                  </div>
                  <select
                    value={m.role}
                    onChange={(e) => updateTeamMember(m.id, { role: e.target.value as TeamRole })}
                  ...
                  <button type="button" onClick={() => removeTeamMember(m.id)} aria-label={`Remove ${m.name}`} className="pcb-btn flex-none rounded-md p-1.5 text-muted-foreground hover:text-destructive">
                    <Trash2 size={15} />
                  </button>
                </div>
                {teamErrors[m.id] && <p className="mt-1 pl-1 text-[11px] font-medium" style={{ color: RED }}>{teamErrors[m.id]}</p>}
              </div>
            ))}
          </div>
          <button
        ...
        <Modal
          ...
          footer={(
            <>
              ...
              <button
                type="button"
                onClick={() => { flashSaved('Notification preferences saved locally'); close(); }}
                className="pcb-btn h-[34px] rounded-lg border-none bg-navy px-4 text-[12.5px] font-semibold text-white"
              >
                Save changes
              </button>
